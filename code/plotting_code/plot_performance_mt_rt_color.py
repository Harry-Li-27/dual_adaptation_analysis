...
def plot_combined(
    ...
    scatter_kwargs = {"s": 50}
    target_legend_handles = None

    if "target_angle" in df.columns:
        target_angle = pd.to_numeric(df["target_angle"], errors="coerce")
        unique_angles = sorted([float(x) for x in target_angle.dropna().unique()])

        if len(unique_angles) > 0:
            cmap = plt.get_cmap("tab10")
            angle_to_color = {a: cmap(i % 10) for i, a in enumerate(unique_angles)}

            point_colors = [angle_to_color.get(float(v), (0.5, 0.5, 0.5, 1.0)) if pd.notna(v) else (0.5, 0.5, 0.5, 1.0) for v in target_angle.to_numpy()]
            scatter_kwargs["c"] = point_colors

            target_legend_handles = [Line2D([0], [0], marker="o", linestyle="None", markerfacecolor=angle_to_color[a], markeredgecolor=angle_to_color[a], markersize=8, label=f"{a:g}") for a in unique_angles]
        else:
    ...
    ax0.set_ylim(-60, 150)
    leg_main = ax0.legend(loc="upper right")
    if target_legend_handles is not None:
        ax0.legend(handles=target_legend_handles, title="target_angle (°)", loc="upper left", ncol=min(6, len(target_legend_handles)))
    ...
    ax1.scatter(df["trialNum"], mt_clipped, label="Movement Time", **scatter_kwargs)
    ax1.set_ylim(0, 710)
    ...
    ax2.scatter(df["trialNum"], rt_clipped, label="Reaction Time", **scatter_kwargs)
    ax2.set_ylim(0, 710)
...
def main():
    ...
    for day in days:
        ...
        if day == 1:
            break_indices = [510-30]
        else:
            break_indices = [214-30]
